...
- (void) postRequests:(NSArray *)requests async:(BOOL)async
{
    ...
    else
    {
        ...
        [serviceRequest setValue:[NSString stringWithFormat:@"%lu", (unsigned long)payload.length] forHTTPHeaderField:@"Content-Length"];
		...
		if(async)
		{
			...
			NSURLConnection *serviceEndpointConnection = [NSURLConnection alloc];
#endif
